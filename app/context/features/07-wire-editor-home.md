...
- The referenced data helper is absent; add `lib/projects.ts` to load owned projects and shared projects matching verified Clerk email addresses.
- Create accepts an optional validated `roomId` (name slug plus 12 hex characters) and persists it as the project primary key. Omission preserves the cuid default; arbitrary `id` and ownership fields remain ignored. Duplicate room IDs return 409. Renaming preserves the ID.
- Workspace navigation uses `/editor/[projectId]`. Add a membership-checked workspace shell to provide the destination; canvas implementation remains a later feature.
